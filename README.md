...
### 1. 2차 팀 프로젝트 관련 기능 구현

- 로그인시
  - [x] 토큰 생성해서 로컬스토리지 ‘**`token'`** 에 담기
  - [x] 해당 로그인한 아이디를 로컬스토리지 `loginId` 에 담기
  - [x] 사용자아이디의 권한을 찾아서 로컬스토리지 **`role`** 에 담기

## 😃 느낀 점

- 해야할것은 너무너무 많은데….. 아직 리액트가 많이 어렵다 … ㅠㅠ
- 익숙해지면 바닐라 js 보다 더 편할거라던데 아직 헷갈리는 점이 많다
...
// 3. useEffect 안의 코드 실행 전에 항상 실행 , unmount 시 1회 실행
useEffect(() => {
  return () => {
    // 여기에 작성
  };
...
// 5. 특정 상태에서 변경시에만 실행하려면 -> state1이 변경될 때만 실행
useEffect(() => {
  실행할코드;
}, [state1]);
```
...
- [x] 리액트 dom을 이용하여 헤더에 마이페이지 기능 넣어서 이동처리

## 🎯 배운것

1. 피그마를 이용한 와이어 프레임 만들기

## 😃 느낀 점

- 오늘부터 본격적으로 시작되는 프로젝트….. 이번에는 스켈레톤 코드도 없어서 거의 무에서 유를 창조해야하는 시간이다 ㅜㅜㅜㅜ
- 너무 걱정되지만 이또한 지나가리라라는 생각, 어떻게든 해내고 만다는 생각으로 해야겠다
- 아니다 그냥 생각하지말고 그냥 해야겠다
...
### 1. 엘리스 실시간 강의 -spa, 라우팅, 비동기 통신 관련

## 🎯 배운 것

### react-bootstrap → [링크](https://react-bootstrap.github.io/)
...
1. 라우팅
   - 사용자의 요청에 따라 특정 페이지로 연결
2. 컴포넌트
   - 컴포넌트들이 모여서 한 페이지를 구성

**SPA의 고려사항**

1. SEO
2. 첫페이지 로딩지연문제
3. 어느 타이밍에 데이터를 가져올것인지
4. 에러 핸들링 방법

### react-router

리액트에서 라우팅기능을 사용하기 위한 라이브러리
...
→ 파일을 읽어오는 동안에 전체 프로세스가 멈추고 데이터를 가져온후 진행
...
### 2. 코딩애플 리액트 강의 시청 및 복습

- 모달창 생성
- 해당 글 클릭시 모달창에 클릭한 내용 제목 뜨게하기

## 😃 느낀 점 및 피드백

- 처음에는 리액트가 좀 쉬운거 같다가도 어렵게 느껴진다.
- 이제 시간이 한달정도 남았다. 좀만 더 힘내자!!!
...
### 2. 엘리스 실시간 강의 및 온라인 강의

- 리액트 스타일링 관련 강의

## 😃 느낀 점 및 피드백

- 월요일부터 너무 피곤했다 ㅜㅜㅜㅜ 그래서 실시간강의는 솔직히 거의 못들었고 , 운동을 다녀와서 겨우 정신차려서 내일 강의를 미리 듣는중이다.
- 월요일강의 녹화본이 올라오면 스피드하게 보고 정리해야지!! 아자!!!
- 알고리즘은 간만에 풀었는데 풀때마다 집중이 잘 되는거같다 ㅎㅎ
...
# 🗓️ 2022.06.15 ~ 18 - (수,목,금,토)

## **✅**한 일

### 1. 엘리스 온라인 강의 및 실강 - react
...
- 함수 내에 선언된 변수처럼 컴포넌트의 렌더링 결과물에 영향을 주는 객체
- 간단하게 state는 변수이다. 하지만 일반변수와는 다르게 값이 변하게 되면 **렌더링**이 일어난다.
...
- 리액트에서 기존에 사용하던 class를 이용한 코드를 작성할 필요없이 state와 여러 react기능을 사용할 수 있도록 만든 라이브러리
- 함수 컴포넌트도 클래스 컴포넌트처럼 사용할수 있게.
- **기본적으로 지원하는 hook**

  1. useState

     컴포넌트의 state(상태)를 관리 할 수있다.

  2. useEffect

     렌더링 이후에 실행할 코드를 만들 수있다.

     어떤 변수가 변경될때마다, 특정 기능이 작동하도록 할 수있다.

## 😃 느낀 점 및 피드백

- 이번주부터 리액트가 시작되었고 전반적으로 우당탕탕 하고있는데 아직까지는 그래도 조금은 할만 한거같다 .
- 갈수록 어려워지겠지만 마지막까지 화이팅!!!
- 복습만이 살길이다!!
...
1. 컴포넌트 기반 구조
   ...
   ui 단위로 캡슐화시켜서 코드를 관리하기때문에 재사용성이 높다.
...
- 낮에 늦잠자서 저녁부터 시작해서 겨우 새벽에 끝난 강의…..
- 정신 차려!!!
...
### 2. 코딩애플 강의 - 리액트 강의 → [링크](https://www.notion.so/4e84c63665344aaa93f343436fd82fe3)

## 😃 느낀 점 및 피드백

- 간만에 실강을 들으니 솔직히 피곤하기도 하고 집중도 안되서 그냥 저냥 넘어갔던 하루 …..
...
- 장바구니 - 진짜 뼈대만 만든 페이지에 bulma css를 적용해서 뼈대만들기
- 코드를 보면서 api구조를 사용한 페이지마다 로그 찍어서 확인하기

## 😃 느낀 점 및 피드백

- 와,….진짜 너무너무 어렵다 ㅜㅜㅜㅜㅜ
- 말로만 api, api하지 직접 겪으니 너무 어렵다
- 근데 이걸 해결하면 너무 기분 좋겠지?
